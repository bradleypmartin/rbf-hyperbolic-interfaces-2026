...
rehearsal, a freeze, and the `talk-2026-09-30` tag (issues demo#18, demo#19).
...
**Part 3 (not in the talk; demo#27, sub-issues demo#28–demo#34).** Started 2026-09-19
...
The 2-D chain (demo#36–demo#42, flat first, curved last) has started: demo#36 added
smooth flat edges to `LayeredMedium2D` and a normal-incidence reference
from the 1-D spectral solver; demo#37 measured the naive RBF-FD baseline
...
unresolved from resolved edges 10× more sharply; demo#38 built the elastic seeds
...
constant-material, jump-limit, residual and conditioning checks; demo#39 built
...
cut the spurious u 5.6× and v below the naive floor. demo#40 ran the flat δ
...
when δ ≤ h. demo#41 sent a plane-wave train in at 26.6° (`oblique_p_wave`,
...
floor with the converted waves in it. demo#42 (curved, the last of the
...
demo#69 (2026-09-20) ran the standing alternative head to head, the same
...
**Manuscript (demo#51, sub-issues demo#52–demo#62).** `paper/` holds the arXiv-ready
...
dirichlet-bridge repos). Merged 2026-09-20: demo#52 scaffold, demo#53 literature
...
unverified citation ships), demo#54 results cache `paper/data/` with the
print-style figures and `tab_*.tex` fragments from
`scripts/paper_figures.py` (`--check` gates byte identity), demo#55 §1–2,
demo#56 §3, demo#57 §4, demo#58 §5; demo#59 §6 drafted (all 14 `tab_2d_*` / `tab_spectra_*`
fragments and the eight 2-D figures placed); demo#69 measured comparison
...
demo#60 §7 drafted (limitations that cross-reference §2.4 and add what
...
verbatim). demo#61 assembly (2026-09-20): every number re-checked against the
...
synchronised with §1.1. demo#62 packaging (2026-09-20): `make_arxiv.py`
...
anchor x_e. One sub-issue per PR, in the dependency order on demo#51.
...
scripts/         drivers that write figures and clips to outputs/;
                 ...
                 figures and tables from paper/data/, demo#54)
