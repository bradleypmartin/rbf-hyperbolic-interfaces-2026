...
def test_aware_is_fourth_order_naive_is_first_order() -> None:
    ...
    aware_rates = np.log2(np.array(aware[:-1]) / np.array(aware[1:]))
    naive_rates = np.log2(np.array(naive[:-1]) / np.array(naive[1:]))
    assert np.all(aware_rates > 3.5), aware_rates
    assert np.all(naive_rates < 1.5), naive_rates
    assert aware[-1] < naive[-1] / 50


def test_thin_layer_double_cross_stays_fourth_order() -> None:
    # Layer width 0.01 is narrower than the 4th-order stencil (4h) for every
    # grid here, so interface stencils straddle both edges at once.
    medium = LayeredMedium(layer_width=0.01)
    ns = [200, 400, 800]
    aware = [_rel_l2_error(n, medium, "aware") for n in ns]
    naive = [_rel_l2_error(n, medium, "naive") for n in ns]
    rates = np.log2(np.array(aware[:-1]) / np.array(aware[1:]))
    assert np.all(rates > 3.5), rates
